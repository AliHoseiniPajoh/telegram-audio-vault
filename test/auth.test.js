...
async function runStorageTests() {
  ...
  // 2.2 Retrieve Track
  const retrieved = await storage.getTrackById(track.id);
  assert.strictEqual(retrieved.title, 'Test Audio Track');
  console.log('  ✔ Retrieved track metadata matches');

  // 2.3 Search Tracks
  const searchResults = await storage.getAllTracks('Test');
  ...
  // 2.4 Create Playlist
  const playlist = await storage.createPlaylist('Study & Chill');
  ...
  const updatedPl = await storage.addTrackToPlaylist(playlist.id, track.id);
  ...
  const deleteResult = await storage.deleteTrack(track.id);
  assert.strictEqual(deleteResult, true, 'Track deletion should return true');
  const plAfterDelete = await storage.getPlaylistById(playlist.id);
